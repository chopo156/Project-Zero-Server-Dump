          ...
          if not Config.DrawTextInsteadOfMarker then
            if doorDist < Config.DrawDist then
              ...
              if doorDist < Config.ActDist then
                ESX.ShowHelpNotification("Pulsa ~INPUT_PICKUP~ para interactuar.")
                if IsControlJustPressed(0, 38) then
            ...
            if doorDist < Config.DrawDist then
              utils.drawText3D(closestDoor,"Pulsa [~r~E~s~] para interactuar.")
              if doorDist < Config.ActDist then
                if IsControlJustPressed(0, 38) then
                  motels.doorLoop(closestDoor,closestZone)
                end
    ...
    while motels.busy do
      ...
      if motels.homeRoom ~= door and motels.ownedRooms[door] then
        if Config.DrawTextInsteadOfMarker then
          utils.drawText3D(door,"Pulsa [~r~G~s~] para entrar.\nPulsa [~r~H~s~] para vender.")
        else
          ESX.ShowHelpNotification("Pulsa ~INPUT_DETONATE~ para entrar.\nPulsa ~INPUT_VEH_HEADLIGHT~ para vender.")
        end
      ...
      elseif motels.ownedRooms[door] then
        ...
        if _owner then
          if motels.plyData and motels.plyData.job and motels.plyData.job.name == Config.PoliceJobName then
            if Config.DrawTextInsteadOfMarker then
              utils.drawText3D(door,"Pulsa [~r~G~s~] para tocar la puerta.\nPulsa [~r~H~s~] para tirar la puerta abajo.")
            else
              ESX.ShowHelpNotification("Pulsa ~INPUT_DETONATE~ para tocar el timbre.\nPulsa ~INPUT_VEH_HEADLIGHT~ para tirar la puerta abajo.")
            end
            if IsControlJustPressed(0, 47) then
            ...
            if Config.DrawTextInsteadOfMarker then
              utils.drawText3D(door,"Pulsa [~r~G~s~] para tocar el timbre.")
            else
              ESX.ShowHelpNotification("Pulsa ~INPUT_DETONATE~ para tocar el timbre.")
            end
            if IsControlJustPressed(0, 47) then
          ...
          if zone ~= Config.HomeMotel then
            if Config.DrawTextInsteadOfMarker then
              utils.drawText3D(door,"Pulsa [~r~G~s~] para comprar esta habitacion ($"..Config.Prices[zone]..")")
            else
              ESX.ShowHelpNotification("Pulsa ~INPUT_DETONATE~ para comprar esta habitacion ($"..Config.Prices[zone]..")")
            end
            if IsControlJustPressed(0, 47) then
              motels.buyRoom(door,zone)
            end
          else
            ESX.ShowHelpNotification("No hay interacciones disponibles.")
          end
        end
      end
      Wait(0)
...
motels.hasKnocked = function(door,id)
  ...
  if motels.curRoom.entry and motels.curRoom.entry == door then
    ESX.ShowNotification("Alguien a tocado a la puerta. Usa /invin ID para dejarle entrar. [ID:~g~"..id..'~s~]')
  end
end
...
motels.inviteIn = function(source,args)
  ...
  elseif not args or not args[1] then
    ESX.ShowNotification("Escribe una ID la proxima vez.")
  else
...
motels.inviteInside = function(door,spawn)
  ...
  if dist < 20.0 then
    Citizen.CreateThread(function(...)
      ...
      ESX.ShowNotification("Has sido invitado dentro. Pulsa [~g~H~s~] para aceptar.")
...
motels.getClosestAction = function(room)
  ...
  closest = "exit"
  actText = "Pulsa "..(Config.DrawTextInsteadOfMarker and "[~r~E~s~]" or "~INPUT_PICKUP~").." para salir."

  local inventDist = utils.vecDist(room.inventory, pos)
  if not closestDist or inventDist < closestDist then
    closestDist = inventDist
    closest = "inventory"
    actText = "Pulsa "..(Config.DrawTextInsteadOfMarker and "[~r~E~s~]" or "~INPUT_PICKUP~").." para abrir el inventario."
  end
  ...
  if not closestDist or wardrobeDist < closestDist then
    ...
    closest = "wardrobe"
    actText = "Pulsa "..(Config.DrawTextInsteadOfMarker and "[~r~E~s~]" or "~INPUT_PICKUP~").." para acceder a tu armario."
  end
...
motels.buyRoom = function(door,zone)
  ESX.TriggerServerCallback('motels:tryBuy', function(canBuy,price)
    if canBuy then
      ...
      ESX.ShowNotification("Has comprado esta habitacion.")
    else
      ESX.ShowNotification("No puedes permitirte esta habitacion.")
    end
  end,zone,door)
end
...
motels.sellRoom = function(door,zone)
  ...
  ESX.ShowNotification("Has vendido la habitacion.")
...
motels.wardrobeMenu = function()     
  local elements = {}
  elements[1] = {label = "Cambiar Ropa",val="Change"}
  elements[2] = {label = "Borrar Outfit",val="Delete"}
