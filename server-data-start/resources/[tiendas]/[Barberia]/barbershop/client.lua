...
function OpenBarbershopMenu()
    ...
    table.insert(elements, { ["label"] = "Corte de Pelo " .. Config.hairCost .. "$", value = 'hairstyle'})
    table.insert(elements, { ["label"] = "Barberia " .. Config.beardCost .. "$", value = 'beardstyle'})
    table.insert(elements, { ["label"] = "Estiliza tus cejas " .. Config.eyebrowCost .. "$", value = 'eyebrowstyle'})
    table.insert(elements, { ["label"] = "Pagar " .. cost .. "$ y Irse", value = 'pagarysalir'})


	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'barberia', {
            title = _U('menu_title'),
            align = 'top-right',
			elements = elements,
		},
		function(data, menu)
			...
			menu.close()
			if(data.current.value == 'hairstyle') then
				menu.close()
                Citizen.Wait(300)
                barberMenu('hairstyle')
			end
			if(data.current.value == 'beardstyle') then
				menu.close()
                Citizen.Wait(300)
                barberMenu('beardstyle')
			end
			if(data.current.value == 'eyebrowstyle') then
				menu.close()
                Citizen.Wait(300)
                barberMenu('hairstyle')
            end
            if(data.current.value == 'pagarysalir') then
				FreezeEntityPosition(GetPlayerPed(-1), false)
                viewangle = false
                started = false
                disableUI = false
