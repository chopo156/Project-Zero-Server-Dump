...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_vehiclescrap:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_vehiclescrap:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_vehiclescrap:sendCategories')
AddEventHandler('esx_vehiclescrap:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_vehiclescrap:sendVehicles')
AddEventHandler('esx_vehiclescrap:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[esx_vehiclescrap] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[esx_vehiclescrap] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_vehiclescrap:buyVehicle', function(success)
					if success then
						IsInShopMenu = false
						menu2.close()
						menu.close()
...
AddEventHandler('esx_vehiclescrap:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_vehiclescrap:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Larry - 2ª Mano")
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_vehiclescrap:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_vehiclescrap:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_vehiclescrap:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_vehiclescrap:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
