...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_vehicleluxe:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_vehicleluxe:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_vehicleluxe:sendCategories')
AddEventHandler('esx_vehicleluxe:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_vehicleluxe:sendVehicles')
AddEventHandler('esx_vehicleluxe:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[esx_vehicleluxe] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[esx_vehicleluxe] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_vehicleluxe:buyVehicle', function(success)
					if success then
						IsInShopMenu = false
						menu2.close()
						menu.close()
...
AddEventHandler('esx_vehicleluxe:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_vehicleluxe:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Concesionario de Lujo")
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_vehicleluxe:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_vehicleluxe:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_vehicleluxe:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_vehicleluxe:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
