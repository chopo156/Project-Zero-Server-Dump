...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_vehiclebike:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_vehiclebike:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_vehiclebike:sendCategories')
AddEventHandler('esx_vehiclebike:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_vehiclebike:sendVehicles')
AddEventHandler('esx_vehiclebike:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[esx_vehiclebike] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[esx_vehiclebike] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_vehiclebike:buyVehicle', function(success)
					if success then
						IsInShopMenu = false
						menu2.close()
						menu.close()
...
AddEventHandler('esx_vehiclebike:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_vehiclebike:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Motocicletas Sanders")
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_vehiclebike:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_vehiclebike:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_vehiclebike:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_vehiclebike:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
