...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[esx_vehiclebike] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM vehicle_categories_bike', {}, function(_categories)
		categories = _categories

		MySQL.Async.fetchAll('SELECT * FROM vehicles_bike', {}, function(_vehicles)
			vehicles = _vehicles
			...
			TriggerClientEvent('esx_vehiclebike:sendCategories', -1, categories)
			TriggerClientEvent('esx_vehiclebike:sendVehicles', -1, vehicles)
		end)
	end)
end)
...
ESX.RegisterServerCallback('esx_vehiclebike:getCategories', function(source, cb)
	cb(categories)
end)

ESX.RegisterServerCallback('esx_vehiclebike:getVehicles', function(source, cb)
	cb(vehicles)
end)

ESX.RegisterServerCallback('esx_vehiclebike:buyVehicle', function(source, cb, model, plate)
...
ESX.RegisterServerCallback('esx_vehiclebike:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_vehiclebike:isPlateTaken', function(source, cb, plate)
...
-- 	print('[esx_vehiclebike] [^2INFO^7] Paying rent cron job started')
...
-- 		print(('[esx_vehiclebike] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
-- 	end)
-- end
