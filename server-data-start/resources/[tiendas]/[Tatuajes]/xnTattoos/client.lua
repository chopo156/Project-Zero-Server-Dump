...
Citizen.CreateThread(function()
	...
	for k, v in pairs(Config.Shops.tattoo.Locations) do
...
RegisterNetEvent('xnTattoos:getTattoos')
AddEventHandler('xnTattoos:getTattoos', function()
	ResetSkin()
end)
function ResetSkin()
...
function OpenTattooShop()
	...
	GetNaked()
	ReqTexts("TAT_MNU", 9)
	TriggerEvent('new_hud:hideHUD', false)
end
...
function CloseTattooShop()
	...
	opacity = 1
	ResetSkin()
	TriggerEvent('new_hud:hideHUD', true)
	return true
end
...
function IsInTattooZone(coords)
    RegularShop = Config.Shops.tattoo.Locations
...
Citizen.CreateThread(function()
    ...
    while true do 
		...
		if not IsMenuOpen() then
			for k, v in pairs(Config.Shops.tattoo.Locations) do
				if GetDistanceBetweenCoords(coords, Config.Shops.tattoo.Locations[k].x, Config.Shops.tattoo.Locations[k].y, Config.Shops.tattoo.Locations[k].z, true) < 3.0 then
					CanSleep = false
					ESX.Game.Utils.DrawText3D(vector3(Config.Shops.tattoo.Locations[k].x, Config.Shops.tattoo.Locations[k].y, Config.Shops.tattoo.Locations[k].z), "Pulsa ~r~[E]~s~ para tatuarte", 0.6)
				end
			end
		end
...
function setupScaleform2(scaleform, message, button, message2, buttons, message3, button2)
    local scaleform = RequestScaleformMovie_2(scaleform)
