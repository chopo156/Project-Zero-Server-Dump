...
RegisterCommand("th",function(source, args)
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola para retener a un rehen!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			drawNativeNotification("No tienes a nadie cerca para ser tu rehen!")
		end 
	-- else
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Pulsa [G] para liberar, [H] para matar")
