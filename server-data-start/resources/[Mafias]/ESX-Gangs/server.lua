...
local function InsertPlayer(ident, gang_name, ranking) 
    ...
    TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, player.firstname .. " " .. player.lastname .. " was set as ^2".. gang_data.label .. ": ".. rank_data.label .."^0.")
...
local function InvitePlayer(ident, gang_name)
    ...
    local player = tplayer[1]
    local gang_data = gangs[gang_name]
    TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, player.firstname .. " " .. player.lastname .. " was ^2accepted^0 into ^2".. gang_data.label .. "^0.")
...
local function FirePlayer(ident)
    ...
    TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, player.firstname .. " " .. player.lastname .. " was ^1removed^0 from ^1".. gang_data.label .. "^0.")
...
local function PromotePlayer(ident)
    ...
    if (getRankData(player.gang, player.gang_rank + 1) ~= nil) then
        TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, "Promoted " .. player.firstname .. " " .. player.lastname .. " to ^2".. getRankData(player.gang, player.gang_rank + 1).label .. "^0.")
        ...
        TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, "Couldn't promote " .. player.firstname .. " " .. player.lastname .. " any higher.")
    end
end
...
local function DemotePlayer(ident)
    ...
    local gang_data = gangs[player.gang]
    if (getRankData(player.gang, player.gang_rank - 1) ~= nil) then
        TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, "Demoted " .. player.firstname .. " " .. player.lastname .. " to ^1".. getRankData(player.gang, player.gang_rank - 1).label .. "^0.")
        MySQL.Sync.execute("UPDATE users SET gang_rank=@gang_rank WHERE identifier=@identifier", {['@gang_rank'] = player.gang_rank - 1, ['@identifier'] = ident})
        if (ESX.GetPlayerFromIdentifier(ident) ~= nil) then
            local xPlayer = ESX.GetPlayerFromIdentifier(ident)
            if (xPlayer.source ~= nil) then
        ...
        TriggerClientEvent("chatMessage", -1, "[^1ESX Gangs^0]", {255,255,255}, "Couldn't demote " .. player.firstname .. " " .. player.lastname .. " any lower.")
    end
end
...
local function FinishCapturing(_zone)
    ...
    for k,v in pairs(members) do
        if (dead[k] == nil) then
            if (v == "police") then 
                local f_data = ESX_Factions.GetPlayerData(k)
                if (f_data.faction.type == "police") then
    ...
    if (highest.name == "") then
        TriggerClientEvent("chatMessage", -1, "[^1Turf Wars^0]", {255,255,255}, 'The turf "^3'.. zone.Label ..'^0" has ended in a ^3tie^0! No winners this time...')
    else
        ...
        if (highest.name ~= "police") then
            ...
            for k,v in pairs(members) do
                if (v == "police") then 
                    local f_data = ESX_Factions.GetPlayerData(k)
                    if (f_data.faction.type == "police" and rewarded_depts[f_data.faction.name] == nil) then
                        ESX_Factions.AddFactionCash(f_data.faction.name, 'cash', (rewards['cash'] + rewards['dcash']) * 2)
                        rewarded_depts[f_data.faction.name] = true
                    end
                end
            end
        end
        ...
        TriggerClientEvent("chatMessage", -1, "[^1Turf Wars^0]", {255,255,255}, 'The turf "^3'.. zone.Label ..'^0" was captured by ^1'.. winner_label..'^0! (Rewards: '.. amount ..'.')
    end
...
AddEventHandler("esx_gangs:InitializeClient", function() 
    local _source = source
    if (initialized[_source] == nil) then
    ...
    if (isGangLeader(leader, l_gang)) then
        ...
        if (#t_player == 1) then
            ...
            if (l_rank.ranking > player.gang_rank + 1) then
                ...
                TriggerClientEvent("chatMessage", leader, "[^1ESX Gangs^0]", {255,255,255}, "Couldn't promote " .. player.firstname .. " " .. player.lastname .. " any higher.")
            end
        end
    end
end)
...
AddEventHandler("esx_gangs:InvitePlayer", function(ident) 
    ...
    if (i_gang == nil) then
        ...
        TriggerClientEvent("chatMessage", leader, "[^1ESX Gangs^0]", {255,255,255}, "That person is already in another gang.")
    end
end)
    ...
    if (isGangLeader(leader, l_gang)) then
        ...
        if (#t_player == 1) then
            ...
            if (l_rank.ranking > player.gang_rank and 0 <= player.gang_rank - 1) then
                ...
                TriggerClientEvent("chatMessage", leader, "[^1ESX Gangs^0]", {255,255,255}, "Couldn't demote " .. player.firstname .. " " .. player.lastname .. " any lower.")
            end
        end
    end
end)
...
AddEventHandler("esx_gangs:zoneInteracted", function(_zone)
    ...
    local onduty = ESX_Factions.GetPlayerData(_source).onduty
    if (zones_waiting[_zone] == nil or zones_waiting[_zone] <= 0) then
        if (zone ~= nil and (gang ~= nil or onduty)) then
            if (not zone.capturing) then 
                zone.capturing = true
                zone.members = {}
                zone.deadmembers = {}
                zone.timer = Config.CaptureTimer
                TriggerClientEvent("chatMessage", _source, "[^1Turf Wars^0]", {255,255,255}, 'The turf "^3'.. zone.Label ..'^0" is now being ^1captured^0. It will be over in ^1'.. Config.CaptureTimer ..' second(s)^0.')
                zones[_zone] = zone
                TriggerClientEvent("esx_gangs:UpdateZones", -1, zones)
            end
        else
            TriggerClientEvent('esx:showNotification', _source, "~r~You are not in a gang or faction.~w~")
        end
    else
...
RegisterCommand("setgang", function(src, args, raw)
    ...
    if (isAdmin(source)) then 
        ...
        if (target ~= nil and gang ~= nil and rank ~= nil) then 
            ...
            TriggerClientEvent("chatMessage", source, "[^1ESX Gangs^0]", {255,255,255}, "^1USAGE: ^3/setgang [player_id] [gang_name] [#rank]")
        end
    else
        TriggerClientEvent("chatMessage", source, "[^1ESX Gangs^0]", {255,255,255}, "^1You don't have permission to use this command.")
    end
end, false)
