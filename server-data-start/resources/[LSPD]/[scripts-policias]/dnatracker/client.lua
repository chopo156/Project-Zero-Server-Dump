...
function MFD:PositionUpdate()
  ...
  while true do
    Citizen.Wait(0)
    tick = tick + 1
    if tick % 200 == 0 then ESX.TriggerServerCallback('DnaTracker:GetJob', function(job) self.CurJob = job.label; end); end
    if self.CurJob == self.AmbulanceJob or self.CurJob == self.PoliceJob then
      ...
      if distA and distA < self.DrawTextDist then
        drawing = "ammo"
        self:DrawText3D(self.AmmoAnalyzePos.x,self.AmmoAnalyzePos.y,self.AmmoAnalyzePos.z, "Presiona [~g~E~s~] para analizar la munición.")
      elseif distB and distB < self.DrawTextDist then
        drawing = "blood"
        self:DrawText3D(self.DNAAnalyzePos.x,self.DNAAnalyzePos.y,self.DNAAnalyzePos.z, "Presiona [~g~E~s~] analizar la sangre.")
      end
      if drawing then
        if (IsControlJustPressed(0, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) then
          if drawing == "ammo" then TriggerEvent('DnaTracker:AnalyzeAmmo')
          elseif drawing == "blood" then TriggerEvent('DnaTracker:AnalyzeDNA')
          end
        end
      end
    end
...
function MFD:HandleDrawText(plyPed)
  ...
  if closestDist and closestDist < 5.0 then
    local str = ''
    if closest.obj == self.BloodObject then str = "[ ~r~DNA~s~ ] : Sangre Humana : Presiona [~r~ E ~s~] para tomar una muestra." else str = "[ ~r~EVIDENCIA ~s~] : Munición : Presiona [~r~ E ~s~] para cogerlo."; end 
    ...
    if (IsControlJustPressed(1, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) and (GetGameTimer() - self.timer) > 150 then
      self.timer = GetGameTimer()
      ESX.TriggerServerCallback('DnaTracker:PickupEvidenceS', function(canPickup)
        if not canPickup then ESX.ShowNotification("No puedes coger mas de eso.")
        else
...
AddEventHandler('DnaTracker:PickupEvidenceC', function(evidence) MFD:PickupEvidence(evidence); end)
...
AddEventHandler('DnaTracker:AnalyzeDNA', function()
  local self = MFD
  ...
  local elements = {
    [1] = { label = "Nombre: " .. self.CurBlood.name},
    [2] = { label = "Tipo: Sangre" },
    [3] = { label = "Calle: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Loc: [ X : " .. math.floor(self.CurBlood.pos.x) .. " ] [ Y : " .. math.floor(self.CurBlood.pos.y) .. "] : [ Z : " .. math.floor(self.CurBlood.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Blood_Info", { title = "Analizador de Sangre", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
...
AddEventHandler('DnaTracker:AnalyzeAmmo', function()
  local self = MFD
  ...
  local elements = {
    [1] = { label = "Arma: " .. newStr},
    [2] = { label = "Tipo: " .. self.CurBullet.weaType },
    [3] = { label = "Calle: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Loc: [ X : " .. math.floor(self.CurBullet.pos.x) .. " ] : [ Y : " .. math.floor(self.CurBullet.pos.y) .. "] : [ Z : " .. math.floor(self.CurBullet.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "AnalyzeAmmo", { title = "Analizador de Municion", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
      ESX.UI.Menu.CloseAll()          
    end,    
...
AddEventHandler('DnaTracker:PlaceEvidenceC', function(pos, obj, name, weapon, weaponType)
