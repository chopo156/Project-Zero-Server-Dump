...
Citizen.CreateThread(function()
    while true do
        ...
        if not isVisible and IsPedInAnyPoliceVehicle(playerPed) and IsControlJustPressed(0, 323) and GetEntitySpeed(playerVeh) < 10.0 then
        ...
        elseif IsControlJustPressed(0, 323) then
...
RegisterNUICallback("getOffender", function(data, cb)
    TriggerServerEvent("mdt:getOffenderDetailsById", data.char_name, data.char_lastname)
    cb('ok')
end)
...
AddEventHandler("mdt:returnVehicleDetails", function(data)
    data.model = GetDisplayNameFromVehicleModel(data.model)
    SendNUIMessage({
