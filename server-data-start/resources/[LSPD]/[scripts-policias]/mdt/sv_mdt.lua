...
AddEventHandler("mdt:getOffenderDetailsById", function(char_name, char_lastname)
	local usource = source
	MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `firstname` = @fname and `lastname` = @lname', {
		['@fname'] = char_name,
		['@lname'] = char_lastname
	}, function(result)
