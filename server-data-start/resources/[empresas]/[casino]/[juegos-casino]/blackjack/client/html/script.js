...
$(function() {
    window.addEventListener('message', function(event) {
        if (event.data.type == "enableui") {
			...
			if (event.data.enable) {			
				...
				$hit.textContent = 'Jugar - Multiplicador x2';
				$stay.textContent = 'Subir apuesta: +100';
				$score.textContent = 'Bajar apuesta -100';
	...
	$hit.addEventListener('click', function () {
		...
		if ($hit.textContent.includes("Jugar - Multiplicador x2") && BlackJack.bet >= 100) {
			$.post("http://blackjack/card", JSON.stringify({}));
			$('#ownedPoints').empty().append(0);
			...
			$.post("http://blackjack/card", JSON.stringify({}));
			// NEW GAME
			$hit.textContent = "Carta";
			coins = coins - BlackJack.bet;
			$('#ownedCoins').empty().append(coins);
			$('#ownedBet').empty().append(BlackJack.bet);
			$stay.textContent = "Plantarse";
			$score.textContent = "Banca tiene 0 puntos";
			$.post("http://blackjack/card", JSON.stringify({}));
			...
			var prom = new Promise(function(res, rej) {
				...
				$score.textContent = "Banca tiene " + BlackJack.getScore(BlackJack.dealerHand.slice(0, 1)) + " puntos.";
			});
			...
			var prom = new Promise(function(res, rej) {
				...
				if (BlackJack.didBust(BlackJack.hand)) {
					...
					BlackJack.Message('Has pedido con ' + BlackJack.getScore(BlackJack.hand) + ' puntos.');
					$hit.textContent = 'Jugar - Multiplicador x2';
					$stay.textContent = 'Subir apuesta: +100';
					$score.textContent = 'Bajar apuesta -100';
					$('#ownedPoints').empty().append(0);
	...
	$stay.addEventListener('click', function () {
		...
		if ($stay.textContent.includes("Subir apuesta: +100")) {
			...
			$score.textContent = "La banca tiene " + BlackJack.getScore(BlackJack.dealerHand) + " puntos.";
			...
			if (BlackJack.getScore(BlackJack.dealerHand) > BlackJack.getScore(BlackJack.hand)) {
				...
				$score.textContent = 'Bajar apuesta -100';
				BlackJack.Lost();
				BlackJack.Message('Has pedido ' + BlackJack.getScore(BlackJack.hand) + ' a ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedBet').empty().append(100);
				queActions(0);
				...
				$score.textContent = 'Bajar apuesta -100';
				BlackJack.Tie();
				BlackJack.Message('Has empatado ' + BlackJack.getScore(BlackJack.hand) + ' a ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedBet').empty().append(100);
				queActions(0);
				...
				$score.textContent = 'Bajar apuesta -100';
				BlackJack.Win();
				BlackJack.Message('Has ganado ' + BlackJack.getScore(BlackJack.hand) + ' a ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedPoints').empty().append(0);
				$('#ownedBet').empty().append(100);
				queActions(0);
			} else {
	...
	$score.addEventListener('click', function () {
		...
		if ($score.textContent.includes("Bajar apuesta -100")) {
			$.post("http://blackjack/bet", JSON.stringify({}));
