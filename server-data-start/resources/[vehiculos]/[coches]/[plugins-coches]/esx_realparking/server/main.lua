		...
		elseif isFound then
			MySQL.Async.fetchAll("SELECT * FROM car_parking WHERE owner = @identifier AND plate = @plate", {
				...
				if type(rs) == 'table' and #rs > 0 then
					...
					MySQL.Async.execute('UPDATE owned_vehicles SET stored = 2, aparcado = 0 WHERE plate = @plate AND owner = @identifier', {
						["@plate"]      = plate,
						["@identifier"] = xPlayer.identifier
					})
					cb({
			...
			MySQL.Async.fetchAll("SELECT * FROM car_parking WHERE owner = @identifier AND plate = @plate", {
				...
				if type(rs) == 'table' and #rs > 0 and rs[1] ~= nil then
					...
					if playerMoney >= fee then
						...
						MySQL.Async.execute('UPDATE owned_vehicles SET stored = 0, aparcado = 0 WHERE plate = @plate AND owner = @identifier', {
							["@plate"]      = plate,
							["@identifier"] = xPlayer.identifier
						})
						cb({
