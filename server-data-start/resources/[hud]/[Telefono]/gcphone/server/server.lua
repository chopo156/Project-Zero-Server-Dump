...
function getPhoneRandomNumber()
    local num = math.random(600000000,699999999)
    return num
end
...
function getSourceFromIdentifier(identifier, cb)
    ...
    for i=1, #users, 1 do
        local xplayer = ESX.GetPlayerFromId(users[i])
...
function getPlayerID(source)
    ...
    local player = jugador.getIdentifier(source) 
    return player
end
...
function addMessage(source, identifier, phone_number, message)
    ...
    if otherIdentifier ~= nil then 
        ...
        getSourceFromIdentifier(otherIdentifier,
        function (osou)
    ...
    if is_valid == true then
        getSourceFromIdentifier(destPlayer, function (srcTo)
            if srcTo ~= nill then
                ...
                exports("SetCallChannel", AppelsEnCours[indexCall].id)
