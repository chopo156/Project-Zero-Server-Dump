...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if messages == nil then
    ESX.ShowNotification('Tienes que escribir algo en el mensaje')
  else  
    if message.owner == 0 then
      local text = 'Nuevo Mensaje'
...
AddEventHandler("gcPhone:acceptCall", function(infoCall, initiator)
  if inCall == false and USE_RTC == false then
    inCall = true
    local infocall = infoCall.id + 300
    exports["mumble-voip"]:SetCallChannel(infocall)
    exports["mumble-voip"]:addPlayerToCall(infocall)
    mumbleCall = infocall
