...
local toggleUID = true


RegisterNetEvent('new_hud:hideHUD')
AddEventHandler('new_hud:hideHUD', function(info)
	local mostrar = info
...
-- Hides TREW UI when it's on Pause Menu or F9
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(0, Keys['F9']) then
			toggleUID = not toggleUID
		end

		if IsPauseMenuActive() or not toggleUID then -- ESC Key
...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(100)
		--TODO: mejorarlo metiendolo en vehicle update
        ...
        if Config.ui.showLocation == true then
		  ...
		  if IsControlPressed(0, Keys['L']) and not toggleMenu then
				SendNUIMessage({ action = 'toggleMenu', value = true })
				toggleMenu = true
		  end

		  if toggleMenu and not IsControlPressed(0, Keys['L']) then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local showLocation = Config.ui.showLocation;
		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			-- if Config.ui.showMinimap == false and toggleUID == true then
			-- 	DisplayRadar(true)
			-- end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		TriggerServerEvent('new_hud:getServerInfo')
...
RegisterNetEvent('new_hud:updateConnectedPlayers')
AddEventHandler('new_hud:updateConnectedPlayers', function(connectedPlayers)
	UpdatePlayerTable(connectedPlayers)
end)
...
function UpdatePlayerTable(connectedPlayers)
	...
	SendNUIMessage({ action = 'updatePlayerJobs', id = 'ems', value = ems})
	SendNUIMessage({ action = 'updatePlayerJobs', id = 'police', value = police})
	SendNUIMessage({ action = 'updatePlayerJobs', id = 'taxi', value = taxi})
	SendNUIMessage({ action = 'updatePlayerJobs', id = 'mechanic', value = mechanic})
	SendNUIMessage({ action = 'updatePlayerJobs', id = 'players', value = players})

end


-- Overall Info
RegisterNetEvent('new_hud:setInfo')
AddEventHandler('new_hud:setInfo', function(info)

	SendNUIMessage({ action = 'setText', id = 'job', value = info['job'] })
	SendNUIMessage({ action = 'setText', id = 'jobgrade', value = info['jobgrade'] })
...
Citizen.CreateThread(function()
	if Config.ui.showVoice == true then
		...
		while true do
			...
			if IsControlJustPressed(1, Keys[Config.voice.keys.distance[1]]) then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and (has_value(vehiclesCars, vehicleClass) == true) then
			...
			TriggerEvent('new_hud:setCarSignalLights', vehicleSignalIndicator)
		end
...
AddEventHandler('new_hud:setCarSignalLights', function(status)
	...
	TriggerServerEvent('new_hud:syncCarLights', status)

	SetVehicleIndicatorLights(driver, 0, leftLight)
	SetVehicleIndicatorLights(driver, 1, rightLight)
end)

RegisterNetEvent('new_hud:syncCarLights')
AddEventHandler('new_hud:syncCarLights', function(driver, status)
...
RegisterCommand('toggleui', function()
	if not toggleui then
		...
		if (Config.ui.showJob == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'jobgrade' })
		end
		if (Config.ui.showSocietyMoney == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'society' })
		end
