...
function UpdatePlayerTable(connectedPlayers)
	...
	if police > 2 then
		SendNUIMessage({
			...
			jobs   = {ems = ems, police = maxpolice, taxi = taxi, mechanic = mechanic, slaughterer = slaughterer, fueler = fueler, lumberjack = lumberjack, tailor = tailor, reporter = reporter, miner = miner, unemployed = unemployed, estate = estate, cardeal = cardeal, arma = arma, stato = stato, player_count = players}
		})
	else
		SendNUIMessage({
			action = 'updatePlayerJobs',
