  ...
  if v then
    if v.health > 0 then
       if X.Clicks >= Config.ClicksToCutNeeded then 
          X.Clicks = 0
          v.health = v.health - 1
          TriggerServerEvent('reborn_jobs:giveWood',v.data)
          TriggerServerEvent('reborn_jobs:getDataWood',Config.Woods)
        else
          X.Clicks = X.Clicks + 1 
        end
    end
...
X.CraftW = function(item)
  ...
  Citizen.CreateThread(function()
    ...
    TriggerServerEvent('reborn_jobs:craftWood',item)
  end)
end
...
X.OpenProcesser = function()
  local elements = {
	...
	{label = Local.process..Local.rubber, value = "rubber"}
  }
