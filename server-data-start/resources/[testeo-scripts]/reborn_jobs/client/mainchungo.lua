...
Local = {
	...
	goFundition       = "Presiona [E] hacer materiales de parada",
	youCantMine       = "No puedes cavar esta roca.",
	fundition         = "¿Quieres derretir estos materiales?",
	melt              = "Detener",
	mine              = "Mina",
	funditionLingots  = "compacto",



	pine              = "Pino",
	treeOf            = "Árbol ",
	woodProcesser     = "¿Quieres procesar esta madera?",
	goProcesser       = "Presiona E para procesar",
	process           = "Proceso ",
	sawmill           = "Aserradero",
...
-----------------------
Config.Blips = {
    mine = {title=Local.mine, colour=46, id=486, x = 2952.71, y = 2790.25, z = 5.76},
   fundition = {title=Local.funditionLingots, colour=46, id=4836, x = 11310.03, y = -20308.15, z = 331.06-1},
...
Citizen.CreateThread(function()
	...
	while true do
	    ...
	    if distanceMine > Config.DistanceToRemoveThePickaxe then
	    	if GetCurrentPedWeapon(GetPlayerPed(-1),"WEAPON_STONE_HATCHET",true) then
	        	RemoveWeaponFromPed(GetPlayerPed(-1),"WEAPON_STONE_HATCHET")
	    	end
	    else
	    	X.CanPick(coords)
	    	if distanceMine < 50 then
	        	...
	        	if distanceMine < 1.5 then
	          		...
	          		if IsControlJustReleased(1,38) then
	              		GiveWeaponToPed(GetPlayerPed(-1),"WEAPON_STONE_HATCHET",1,false,true)
	          		end
	        	end
	      	end
	    end
