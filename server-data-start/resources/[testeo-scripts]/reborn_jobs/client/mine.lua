  ...
  if v then
    ...
    if v.health > 0 then
       if X.Clicks >= Config.ClicksToPickNeeded then 
          X.Clicks = 0
          v.health = v.health - 1
          TriggerServerEvent('reborn_jobs:giveMineral',v.data)
          TriggerServerEvent('reborn_jobs:getData',Config.Rocks)
        else
          X.Clicks = X.Clicks + 1 
        end
    end
...
X.Craft = function(item)
  ...
  Citizen.CreateThread(function()
    ...
    X.IsFunding = false
    TriggerServerEvent('reborn_jobs:craft',item)
  end)
end

X.CanPick = function(coords)
  for i,v in pairs(Config.Rocks) do
    if GetDistanceBetweenCoords(coords.x,coords.y,coords.z,v.x,v.y,v.z) < 15 then
    ...
    if GetCurrentPedWeapon(GetPlayerPed(-1),"WEAPON_Tomahawk",true) then
...
X.OpenFundition = function()
  ...
  local elements = {
    ...
    {label = Local.melt..Local.copper, value = "copper"},
    {label = Local.melt..Local.aluminium, value = "aluminium"},
  }
